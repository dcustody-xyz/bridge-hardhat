...
contract ProgrammableTokenTransfers is CCIPReceiver, OwnerIsCreator {
    ...
    bytes private lastReceivedText; // Store the last received text.
        ...
        onlyWhitelistedSenders(abi.decode(any2EvmMessage.sender, (address))) // Make sure the sender is whitelisted
    {
        ...
        (address dest, address destToken, uint destMinAmount) = abi.decode(any2EvmMessage.data, (address, address, uint)); // abi-decoding of the sent text
        // address srcToken = any2EvmMessage.destTokenAmounts[0].token;
        // uint srcAmount = any2EvmMessage.destTokenAmounts[0].amount;

        // make swap here (transfer is just for test)
        IERC20(destToken).transfer(dest, destMinAmount);
    ...
    function encodeToBytes(address _dest, address _token, uint _amount) public pure returns (bytes memory) {
        return abi.encode(_dest, _token, _amount);
    }
}
